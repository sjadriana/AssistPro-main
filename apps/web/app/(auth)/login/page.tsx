...
import { FlouaIcon, LogoFull } from "@/components/brand/logo"
...
const socialProofs = [
  { name: "Camila R.", role: "Personal Trainer", text: "Economizo 2h por dia só com os lembretes automáticos." },
  { name: "Rafael M.", role: "Prof. de Tênis", text: "Meus alunos adoram confirmar pelo WhatsApp." },
  { name: "Juliana S.", role: "Fisioterapeuta", text: "As cobranças automáticas acabaram com a inadimplência." },
]
...
export default function LoginPage() {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-background">
      ...
      <aside className="relative hidden flex-col justify-between overflow-hidden bg-[oklch(0.22_0.08_293)] p-10 lg:flex lg:w-[52%]">
        {/* Halo de fundo */}
        <div aria-hidden="true" className="pointer-events-none absolute inset-0">
          <div className="absolute -top-32 -left-24 h-[480px] w-[480px] rounded-full bg-primary/30 blur-[120px]" />
          <div className="absolute bottom-0 right-0 h-[360px] w-[360px] rounded-full bg-[oklch(0.55_0.25_310)]/20 blur-[100px]" />
        </div>

        {/* Logo */}
        <div className="relative">
          <LogoFull dark iconClassName="size-10" className="gap-3" />
        </div>

        {/* Tagline central */}
        <div className="relative flex flex-col gap-6">
          <p className="text-4xl font-bold leading-snug text-white text-balance">
            Menos tempo{" "}
            <span className="text-[oklch(0.78_0.18_293)]">organizando.</span>
            <br />
            Mais tempo{" "}
            <span className="text-[oklch(0.78_0.18_293)]">atendendo.</span>
          </p>
          <p className="max-w-sm text-base leading-relaxed text-white/60">
            Agenda online, confirmações e cobranças automáticas pelo WhatsApp — tudo num único lugar.
          </p>
          ...
          <div className="flex flex-col gap-3">
            {socialProofs.map((proof, i) => (
              ...
              >
                <p className="text-sm text-white/90">&ldquo;{proof.text}&rdquo;</p>
        ...
        {/* Rodapé */}
        <p className="relative text-xs text-white/30">© 2025 Floua. Todos os direitos reservados.</p>
      </aside>
      ...
      <main className="flex flex-1 flex-col items-center justify-center overflow-y-auto px-6 py-10">
        <div className="w-full max-w-sm">
          ...
          <button
            type="button"
            className="mt-6 flex w-full items-center justify-center gap-3 rounded-xl border border-border bg-card px-4 py-3 text-sm font-semibold text-foreground transition-colors hover:bg-secondary"
          >
            <GoogleIcon />
